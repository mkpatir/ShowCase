...
import com.mkpatir.showcase.internal.extension.orZero
...
data class EditorShopViewState(
    ...
    fun getProductImage2Url(): String = if (item.popularProducts?.size.orZero() < 2 || item.popularProducts?.get(1)?.images.isNullOrEmpty()) "" else item.popularProducts?.get(1)?.images?.get(0)?.thumbnailPhoto?.url.orEmpty()

    fun getProductImage3Url(): String = if (item.popularProducts?.size.orZero() < 3 || item.popularProducts?.get(2)?.images.isNullOrEmpty()) "" else item.popularProducts?.get(2)?.images?.get(0)?.thumbnailPhoto?.url.orEmpty()
